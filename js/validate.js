function validate(event) {
  ...
  if (isNaN(lat) || lat < -90 || lat > 90) {
    let text = document.createTextNode("must be a valid number between -90 and 90");
    lat_err.appendChild(text);
    return false;
  }
  else if (isNaN(lon) || lon < -180 || lon > 180) {
    let text = document.createTextNode("must be a valid number between -180 and 180");
    ...
    if (!isNaN(lat) && (lat >= -90 && lat <= 90)) {
      ...
      let txt = document.createTextNode("must be a valid number between -90 and 90");
      lat_err.appendChild(text);
      return false;
    }
  }
